...
# Create Virtualenv
# -----------------------
ENV VENV_ROOT=/openstax-built/venv
...
ENV NVM_ROOT=/openstax-built/nvm
RUN mv /root/.nvm $NVM_ROOT
ENV PATH=$PATH:$NVM_ROOT/versions/node/v$NODE_VERSION/bin/
...
COPY --from=build-xhtml-validator-stage $PROJECT_ROOT/xhtml-validator/build/libs/xhtml-validator.jar /openstax-built/xhtml-validator.jar
COPY --from=build-mathify-stage $PROJECT_ROOT/mathify/ /openstax-built/mathify/
...
COPY --from=build-python-stage $VENV_ROOT/ $VENV_ROOT/
