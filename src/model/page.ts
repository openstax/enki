...
export class PageFile extends XMLFile<PageData> {
    protected async innerParse(pageFactory: Factory<PageFile>, resourceFactory: Factory<ResourceFile>) {
        ...
        const pageLinks = dom(doc).$$('//h:a[not(starts-with(@href, "http:") or starts-with(@href, "https:") or starts-with(@href, "#"))]').map(a => {
        ...
        return {
            hasMathML: dom(doc).$$('//m:math').length > 0,
            hasRemoteResources: dom(doc).$$('//h:iframe|//h:object/h:embed').length > 0,
            hasScripts: dom(doc).$$('//h:script').length > 0,
            pageLinks,
            resources
        }
    }
    private resourceFinder(resourceFactory: Factory<ResourceFile>, node: Node, sel: string, attrName: string) {
        return dom(node as ParentNode).$$(sel).map(img => resourceFactory.getOrAdd(assertValue(img.attr(attrName)), this.readPath))
    }
    private resourceRenamer(node: Node, sel: string, attrName: string) {
        ...
        const resources = dom(node as ParentNode).$$(sel)
    ...
    protected transform(doc: Document) {
        ...
        const mathEls = dom(doc).$$('//h:math|//h:math//*')
        mathEls.forEach(el => {
        ...
        dom(doc).$$('//m:math//m:annotation-xml|//h:math//h:annotation-xml').forEach(n => n.remove())
        ...
        dom(doc).$$('//h:script|//h:style').forEach(n => n.remove())
    }
}
